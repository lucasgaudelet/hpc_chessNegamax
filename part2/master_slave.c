...
node_t* next_task(node_t* root) {
	...
	while(root->sons) {
		//printf("[ROOT] n_moves= %d; current_work=%d\n", root->n_moves, root->current_work);
...
void manage_result(result_t* result, node_t** slave_work, node_t* slave) {
	...
	node_t* father = slave->father;
	int i = (int)(slave - father->sons);
	...
	// mise à jour du alpha et nombre de result analysés
	father->result_nb++;
	father->tree.alpha = MAX(father->tree.alpha, child_score);
	
	if(father->result_nb==father->n_moves) { // si tous les fils ont été traités
		if(father->father) manage_result(&father->result, slave_work, father);
	}
...
void master_evaluate(node_t* root)
{
	...
	/* Generation de l'arbre de controle du premier fils */
	root->father = NULL;
	root->current_work = 0;
	root->result_nb = 0;
	root->n_moves = 0;
	...
	for( int ind=0; ind<root->n_moves; ind++) {
		/* construction du sous-arbre du fils */
		son = root->sons[ind];
		son.father = NULL;
	
		play_move(&root->tree, root->moves[ind], &son.tree);
		generate_control_tree( &son, 0);
		
		/* calcul du fils */
		for(int i=1; i<np; i++) {
			node_t* task = next_task(&son);
		...
		while( son.result_nb < son.n_moves ) {
			MPI_Recv(&result, 1, MPI_RESULT_T, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
			manage_result( &result, slave_work, slave_work[status.MPI_SOURCE]);
		...
		int child_score = -son.result.score;
		
		if ( child_score > root->result.score ) {

			root->result.score = child_score;
			root->result.best_move = root->moves[0];

				root->result.pv_length = son.result.pv_length + 1;
				for(int j = 0; j < son.result.pv_length; j++)
					root->result.PV[j+1] = son.result.PV[j];
				root->result.PV[0] = root->moves[0];
		}
		root->tree.alpha = MAX(root->tree.alpha, child_score);
		son.father = root;	root->current_work++;	root->result_nb++;
